...
arguments="$*"
case "${arguments}" in
  "compose -f docker-compose.yml -f docker-compose.incident.yml --profile incident-worker up -d --no-deps --wait --wait-timeout "*" incident-api incident-worker")
    exit "${FAKE_UP_EXIT:-0}"
    ;;
  "compose -f docker-compose.yml -f docker-compose.incident.yml ps -q "*)
    service="${arguments##* }"
  ...
  "inspect --format "*)
    ...
    ;;
  "compose -f docker-compose.yml -f docker-compose.incident.yml exec -T incident-api python -m app.healthcheck ready")
    exit "${FAKE_API_READY_EXIT:-0}"
    ;;
  "compose -f docker-compose.yml -f docker-compose.incident.yml exec -T incident-worker python -c "*)
    exit "${FAKE_LOKI_READY_EXIT:-0}"
    ;;
  "compose -f docker-compose.yml -f docker-compose.incident.yml ps incident-api incident-worker loki alloy")
    printf '%s\n' "incident diagnostic status"
    ;;
  "compose -f docker-compose.yml -f docker-compose.incident.yml logs --tail=100 incident-api incident-worker loki alloy")
    printf '%s\n' "incident diagnostic logs"
    ;;
  *)
...
[[ "${failure_output}" == *"incident diagnostic status"* ]]
[[ "${failure_output}" == *"incident diagnostic logs"* ]]
...
echo "Incident analyzer health gate tests passed"
