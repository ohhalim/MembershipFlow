...
compose=(docker compose -f docker-compose.yml -f docker-compose.incident.yml)

print_diagnostics() {
  "${compose[@]}" ps incident-api incident-worker loki alloy || true
  "${compose[@]}" logs --tail=100 incident-api incident-worker loki alloy || true
}

if ! "${compose[@]}" --profile incident-worker up -d --no-deps --wait \
...
for service in incident-api incident-worker loki alloy; do
...
echo "Incident analyzer health gate passed"
